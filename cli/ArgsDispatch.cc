...
namespace tet_weave {
    ...
    TetMesh generate_mesh_from_args(argparse::ArgumentParser& parser) {
        ...
        if (mesh_type == "voxel-grid") {
            ...
            mesh = SimpleMeshGen::generate_sine_mesh(length, axial_scaling, torsion_rad,
                                                     period, amplitude);
            ...
            const std::string para_type = mesh_type.substr(5);
    ...
    int parse_args(int argc, char** argv, argparse::ArgumentParser& parser) {
        ...
        parser.add_argument("--sine-amplitude")
                .help("amplitude of the sine deformation (mesh-type: sine).")
                .default_value(1.0)
                .nargs(1)
                .scan<'g', double>();
