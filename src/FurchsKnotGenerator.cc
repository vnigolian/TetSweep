...
TetrahedralMesh FurchsKnotGenerator::generate_mesh(int hole_length){

    FurchsKnotGenerator generator;
    ...
    default:{
        std::cerr<<" unhandled Furch's knot hole length"<<std::endl;
    }
    }
...
TetrahedralMesh FurchsKnotGenerator::generate_voxel_grid_mesh(int width, int height, int depth){

    FurchsKnotGenerator generator(width, height, depth);
...
FurchsKnotGenerator::FurchsKnotGenerator(int width,
                                         int height,
                                         int depth)
    : width_(width), height_(height), depth_(depth){
    std::cout<<" - setup Furch's knot gen of dimensions "<<width_<<" x "<<height_<<" x "<<depth_<<std::endl;
}



void FurchsKnotGenerator::generate_five_tet_voxel(){
...
void FurchsKnotGenerator::generate_vertices(){
...
void FurchsKnotGenerator::generate_cube(int x, int y, int z,
                                        bool orientation){
...
void FurchsKnotGenerator::dig_hole(int start_x, int start_y, int start_z,
...
bool FurchsKnotGenerator::is_digged(int x, int y, int z){
...
int FurchsKnotGenerator::coordinates_to_corner_vertex_idx(int x, int y, int z){

    return x + y * (width_ + 1) + z * (width_ + 1) * (height_ + 1);
}


int FurchsKnotGenerator::coordinates_to_voxel_idx(int x, int y, int z){
