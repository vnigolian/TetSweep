...
namespace tet_weave {
    ...
    public:
        ...
        /// generic ctor with other arithmetic types
        template<typename X, typename Y, typename Z>
            requires std::is_arithmetic_v<X> && std::is_arithmetic_v<Y> && std::is_arithmetic_v<Z>
        Vec3d(X x, Y y, Z z) : x_(static_cast<double>(x)), y_(static_cast<double>(y)), z_(static_cast<double>(z)) {
        }
        ...
        Vec3d rotate(Vec3d axis, double angle_rad) const {
